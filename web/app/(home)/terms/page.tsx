  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      <div
        ...
        style={{ background: "radial-gradient(circle, oklch(0.78 0.15 175 / 0.4), transparent 70%)" }}
      />
      <SiteHeader />
      <main className="mx-auto max-w-3xl px-6 py-16 sm:py-24">
        ...
        <div className="glass rounded-3xl p-8 sm:p-12 shadow-elegant">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-gradient-mint grid place-items-center shadow-mint">
              <ScrollText className="h-5 w-5 text-background" />
            </div>
            <span className="text-xs uppercase tracking-wider font-mono text-muted-foreground">
              {t.common.lastUpdated}: {t.terms.date}
            </span>
          </div>
          ...
          <div className="mt-10 space-y-8">
            {t.terms.sections.map((s: { h: string; p: string }, i: number) => (
              <section key={i} className="relative pl-5 border-l-2 border-border hover:border-mint/60 transition">
