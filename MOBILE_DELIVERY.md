# Mobile delivery — Expenn

How the Expenn Traveler app gets from a branch to a device. Four independent pipelines,
...
> `mobile/fastlane/Fastfile` — the build steps around it stay as they are.
...
Pushing to `dev` with changes under `mobile/` runs `mobile-android-dev.yml`:
...
| `PROD_API_URL` | `https://api.expenn.com` | API the release build points at |
...
| `ANDROID_PACKAGE_NAME` | `com.osascloud.expenn` | Android application ID |
| `IOS_BUNDLE_IDENTIFIER` | `com.expenn.traveler` | iOS bundle ID |
...
1. Firebase console → add an Android app with package `com.osascloud.expenn`
   (and an iOS app with bundle ID `com.expenn.traveler` if you enable the iOS lane).
...
`mobile/scripts/set-native-build-number.mjs` patches `mobile/app.json`
immediately before `expo prebuild`, and the workflow reverts it afterwards:

```yaml
- name: Set native build number
  run: node scripts/set-native-build-number.mjs "$GITHUB_RUN_NUMBER"
...
- name: Discard local version bump
  if: always()
  run: git checkout -- app.json
```

The committed `app.json` therefore never carries a specific run's build number.
The **marketing version** (`expo.version`, currently `0.1.0`) is *not* touched —
...
CI installs inside `mobile` (`pnpm install --frozen-lockfile`), keyed on
...
| Symptom | Cause |
| --- | --- |
| `Could not find scheme ExpennTraveler` | `expo prebuild` names the Xcode project after the app name. If the app is renamed, set the `IOS_SCHEME` variable — no code change needed |
...
| `bundler: command not found: fastlane` | `Gemfile`/`Gemfile.lock` out of sync — run `bundle install` in `mobile/` and commit the lockfile |
...
mobile/
  ...
  scripts/set-native-build-number.mjs   CI build-number patcher
```
